...
unsigned release-candidate boundary. The current WP-15 source adds endpoint admission, loaded-driver
trust, runtime-module authorization, and typed target observations; commit-bound VM and Driver
Verifier acceptance remains pending. Production certification, keys, and distribution remain
external.
...
| Endpoint admission | The service requires one complete, identity-correlated production scan, evaluates the frozen loaded-driver inventory by signature, hash, and blocked family, and obtains backend acknowledgement before opening launcher IPC | Driver-free and source-level coverage is present; a new commit-bound VM/Verifier campaign is pending |
| Runtime modules | The signed game manifest contains a bounded runtime-module allowlist and an explicit trusted-Windows-module policy; the service evaluates current path, hash, and Windows trust for observed images | Hostile manifest and evaluator units are present; mapped-file identity and representative middleware/JIT tuning remain pending |
| Target observations | A bounded worker emits typed findings for executable private/mapped memory, embedded PE headers, direct-syscall stubs, thread start/RIP/stack/debug-register state, instrumentation callbacks, and lifecycle events | Driver-free helper and policy coverage is present; runtime acceptance remains pending and skipped coverage stays explicit |
...
recorded Windows runtime claim. WP-15 changes privileged endpoint behavior and therefore requires a
fresh campaign before its source state becomes runtime evidence.
